...
import eu.rcauth.delegserver.oauth2.exceptions.IncompleteAttributeSetException;
...
public class UniqueAttrGenerator {
    ...
    public UniqueAttrGenerator(String[] attrSources, Logable logger) {
        ...
        // Note that the uniqueAttrSourceList is only used when calling toString() to print the object
        for ( String source : uniqueAttrSources ) {
            uniqueAttrSourceList = ( uniqueAttrSourceList.isEmpty() ) ? source : uniqueAttrSourceList + "," + source;
        }
